...
module avg_pool_2x2_banked_internal_fixed #(
    ...
    // Registered - latched at start_pool
    //
    // FIX A: (* keep = "true" *) on out_width_r
    //   Vivado prunes out_width_r[1] because its bit-level liveness analysis
    //   does not trace the bit through the address arithmetic back to a primary
    //   output. This corrupts pooling window addressing for any input width
    //   whose halved value has bit[1] set (e.g. input_width=12 -> out_width=6
    //   = 3'b110, bit[1]=1). Keep preserves all bits.
    //
    // FIX B: (* keep = "true" *) on in_channels_r
    //   all_done compares ch_cnt == in_channels_r-1.  Vivado can prune low
    //   bits of in_channels_r if it decides the subtraction result is
    //   sufficiently constrained by context.  Keep is cheap insurance.
    // =========================================================================
    logic [$clog2(MAX_IN_CHANNELS+1)-1:0]  in_channels_r;
    logic [$clog2(MAX_INPUT_WIDTH+1)-1:0]  input_width_r;
    logic [$clog2(MAX_OUT_HEIGHT+1)-1:0]   out_height_r;
    (* keep = "true" *) logic [$clog2(MAX_OUT_WIDTH+1)-1:0]    out_width_r;   // FIX A
    logic [$clog2(MAX_CHANNEL_SIZE+1)-1:0] channel_size_r;
    logic [$clog2(MAX_TOTAL_SIZE+1)-1:0]   total_size_r;
    ...
    // =========================================================================
    // FIX C: rd_channel internal register with (* keep = "true" *)
    //
    // Vivado prunes rd_channel[7] (the MSB) because in this design the
    // maximum channel count (MAX_IN_CHANNELS=120) fits in 7 bits, so the
    // synthesiser considers bit[7] structurally zero and removes its FF.
    // However the port is declared [7:0] and the parent module (final.sv)
    // uses rd_channel to compute pool_out_addr = pool_channel * pool_out_w *
    // pool_out_h + ..., so a pruned MSB corrupts the address for designs that
    // are later re-parameterised with more channels, and more importantly the
    // pruning warning itself indicates the synthesis netlist diverges from RTL.
    //
    // Fix: drive the output port through an internal (* keep = "true" *)
    // register so Vivado cannot eliminate any bit.
    // =========================================================================
    (* keep = "true" *) logic [7:0] rd_channel_int;  // FIX C
    (* keep = "true" *) logic [7:0] rd_row_int;
    (* keep = "true" *) logic [7:0] rd_col_int;

    assign rd_channel = rd_channel_int;
    assign rd_row     = rd_row_int;
    assign rd_col     = rd_col_int;
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            rd_data           <= '0;
            rd_channel_int    <= '0;   // FIX C: drive internal register
            rd_row_int        <= '0;
            rd_col_int        <= '0;
            val_tl            <= '0;
            val_tr            <= '0;
            ...
            case (state)
                ...
                ST_OUTPUT: begin
                    ...
                    rd_channel_int <= 8'(ch_cnt);   // FIX C: write internal reg
                    rd_row_int     <= 8'(row_cnt);
                    rd_col_int     <= 8'(col_cnt);
                    rd_valid       <= 1'b1;
