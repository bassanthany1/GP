...
hw = np.array(read_mem_signed("fc2_output_hex.txt"), dtype=np.int32)
sw = np.array(read_mem_signed("fc2_golden_reference.mem"), dtype=np.int32)
...
print(f"\n{'='*50}")
print(f"Exact:     {np.sum(diff==0)}/84 ({100*np.sum(diff==0)/84:.1f}%)")
print(f"Within ±{TOLERANCE}: {np.sum(diff<=TOLERANCE)}/84({100*np.sum(diff<=TOLERANCE)/84:.1f}%)")
